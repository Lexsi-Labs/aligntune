...
base_config = EvalConfig(
    ...
    dataset_name=DATASET,
    split="train[11000:]",  # fixed: dataset only exposes a "train" split (was "test", which doesn't exist); rows 11000+ are held out from training's train[:50%] slice
...
# fixed: was run_eval(base_config, trainer.dataset_dict) -- trainer.dataset_dict
# only contains the train[:50%] rows used for training, keyed by that literal
# split string, so requesting the held-out "train[11000:]" split against it
# always raised "split not found". Passing None makes run_eval load the
# held-out split fresh from the Hub instead of reusing training's in-memory data.
base_results = run_eval(base_config, None)
...
if trained_model_path is None:
    ...
    trained_config = EvalConfig(
        ...
        # Dataset
        dataset_name=DATASET,
        split="train[11000:]",  # fixed: dataset only exposes a "train" split (was "test", which doesn't exist); same held-out eval split as the base-model eval above
    ...
    # fixed: same trainer.dataset_dict-reuse issue as the base-model eval above
    trained_results = run_eval(trained_config, None)
