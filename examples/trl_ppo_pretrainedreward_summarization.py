# -*- coding: utf-8 -*-
"""TRL_PPO_PretrainedReward_Summarization.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wlbSvQAJv8ZFM2qGD4XlUOKPzZHXstQo
...
# PPO with Pretrained Reward Model - Summarization

**Proximal Policy Optimization** with pretrained reward model for helpful assistant training.

- **Model**: EleutherAI/pythia-1.4b
- **Dataset**: HuggingFaceH4/helpful-base
- **Backend**: TRL
- **Reward Model**: OpenAssistant/reward-model-deberta-v3-large-v2 (pretrained)
- **Metrics**: Reward score, Helpfulness score
- **Comparison**: Base Model vs PPO with Pretrained Reward Model
"""
...
"""## 2. Train PPO with Pretrained Reward Model

**Pretrained Reward Model Approach**:
- Uses OpenAssistant/reward-model-deberta-v3-large-v2
- Pre-trained on human preferences for helpfulness
- No reward model training needed - ready to use!
...
ppo_trainer = create_rl_trainer(
    ...
    split="train[:10000]",  # Increased from 5000
    ...
    # ============================================================================
    num_epochs=1,
    batch_size=16,                      # Increased from 4 (adjust based on GPU)
    mini_batch_size=8,                  # Increased from 1
    gradient_accumulation_steps=1,      # Reduced since batch_size increased
    max_steps=-1,                       # -1 means train for full epoch
    learning_rate=4e-5,              # Optimized for PPO (was 3e-6)
    ...
    # ============================================================================
    use_lora=True,
    lora_r=32,                         # Increased rank (was 64)
    lora_alpha=64,                     # 2x rank (was 64)
