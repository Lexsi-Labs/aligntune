...
Original file is located at
    https://colab.research.google.com/drive/11DrRbG30MUCegZTDXwR9nxEdvKaLfWCb
...
"""SFT Training for Qwen NPC (MobileGameNPC)

Dataset:
  - bebechien/MobileGameNPC (config: martian / venusian)
  - Columns: player (user), alien (assistant)
"""
...
pre = results["pre_training_eval"]
post = results["post_training_eval"]

for metric in pre:
    if metric in post and isinstance(pre[metric], (int, float)):
        delta = post[metric] - pre[metric]
        print(f"{metric}:")
        print(f"  Pre  = {pre[metric]:.6f}")
        print(f"  Post = {post[metric]:.6f}")
        print(f"  Δ     = {delta:+.6f}\n")
