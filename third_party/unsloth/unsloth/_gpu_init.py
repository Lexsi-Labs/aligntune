...
# --- aligntune patch (see third_party/unsloth/PATCH_NOTES.md) ---
# Upstream checks `importlib.metadata.version("unsloth_zoo")` here to guard
# against a stale unsloth_zoo install. That check assumes unsloth_zoo is its
# own separately pip-installed distribution with its own dist-info; here
# it's vendored as part of aligntune's own distribution instead (no separate
# "unsloth_zoo" dist-info exists), so the lookup always raises
# PackageNotFoundError even though the code is present and importable.
# Both are vendored from the same upstream release (2026.7.2) in lockstep,
# so the version-skew this check guards against can't happen here - skip it.
import unsloth_zoo
