...
# Newer `transformers` releases implement `PretrainedConfig` (or fields nested
# within it) using stdlib dataclasses with forward-referenced annotations
# (e.g. a string-quoted "torch.dtype") that aren't resolvable from pydantic's
# schema-generation context. Even `SkipValidation` still builds a serializer
# schema and hits the same unresolved forward ref, so we type this field as
# `Any` for pydantic's purposes (no schema recursion at all) while every
# actual runtime value remains a real `PretrainedConfig` instance.
_PretrainedConfigField = Any
