"""
DPO Algorithm Test Suite - Unsloth Backend Only

Tests DPO with Unsloth backend to avoid Unsloth global patch conflicts
...
'apply_qkv'). See test_dpo_algorithm_trl.py for the TRL backend.
...
def test_unsloth_dpo_available():
    """Test Unsloth DPO is available."""
    from aligntune.backends.unsloth.rl.dpo.dpo import UnslothDPOTrainer
    assert UnslothDPOTrainer.is_available()


def test_dpo_unsloth_registered():
    """Test DPO Unsloth backend is registered in BackendFactory."""
    from aligntune.core.backend_factory import BackendFactory, TrainingType, BackendType, RLAlgorithm

    unsloth_key = (TrainingType.RL, BackendType.UNSLOTH, RLAlgorithm.DPO)
