"""
GRPO End-to-End Test with GSM8K Dataset and Custom Reward Functions - Unsloth Backend

Full pipeline testing for GRPO and variants (GSPO, DAPO, DR-GRPO) with Unsloth backend
Unsloth provides 2-5x speed improvements over standard GRPO training
...
# =============================================================================
# TEST GRPO FULL PIPELINE - UNSLOTH
# =============================================================================

def test_grpo_unsloth_full_pipeline():
    """Test GRPO full pipeline with GSM8K dataset using Unsloth backend."""
    output_dir = tempfile.mkdtemp(prefix="grpo_unsloth_e2e_test_")

    print("\n" + "="*80)
    print("GRPO E2E TEST - UNSLOTH BACKEND - GSM8K DATASET WITH REWARD FUNCTIONS")
    print("="*80)

    # Create trainer
    print("\n[1/5] Creating GRPO trainer with Unsloth...")
    ...
    assert len(trainer.reward_functions) >= 1, \
        f"Expected at least 1 reward, got {len(trainer.reward_functions)}"
...
def test_grpo_variants_unsloth(algorithm):
    """Test GRPO variants with Unsloth backend."""
    output_dir = tempfile.mkdtemp(prefix=f"grpo_{algorithm}_unsloth_test_")

    print(f"\n{'='*80}")
    print(f"UNSLOTH GRPO VARIANT TEST: {algorithm.upper()}")
    print(f"{'='*80}")

    # Create trainer
    print(f"\n[1/3] Creating {algorithm.upper()} trainer with Unsloth backend...")
    trainer = create_rl_trainer(
        model_name=MODEL,
        dataset_name=DATASET,
        config_name=DATASET_CONFIG,
    ...
    print(f"✓ {algorithm.upper()} trainer created with Unsloth backend")
    ...
    print(f"✓ {algorithm.upper()} setup complete")
    ...
    print(f"\n{'='*80}")
    print(f"✓ {algorithm.upper()} (UNSLOTH) TEST PASSED!")
    print(f"{'='*80}")


def test_grpo_unsloth_with_multiple_rewards():
    """Test GRPO with all three reward functions using Unsloth backend."""
    output_dir = tempfile.mkdtemp(prefix="grpo_unsloth_multi_rewards_test_")

    print(f"\n{'='*80}")
    print(f"GRPO TEST (UNSLOTH): Multiple Reward Functions")
    print(f"{'='*80}")

    # Create trainer
    print(f"\nCreating GRPO trainer with 3 rewards (Unsloth)...")
    trainer = create_rl_trainer(
        model_name=MODEL,
        dataset_name=DATASET,
        config_name=DATASET_CONFIG,
    ...
    print(f"✓ GRPO trainer created with 3 rewards (Unsloth)")
    ...
    assert trainer.model is not None, "Model not initialized!"
    assert trainer.train_dataset is not None, "Dataset not loaded!"
    assert len(trainer.reward_functions) >= 1, "Expected reward functions!"
    print(f"✓ GRPO setup complete with {len(trainer.reward_functions)} rewards (Unsloth)")

    # Run training
    print(f"\nRunning GRPO training with multiple rewards (Unsloth)...")
    result = trainer.train()
    assert result is not None, "GRPO training returned None!"
    print(f"✓ GRPO training completed successfully")

    print(f"\n{'='*80}")
    print(f"✓ GRPO MULTI-REWARD (UNSLOTH) TEST PASSED!")
    print(f"{'='*80}")
