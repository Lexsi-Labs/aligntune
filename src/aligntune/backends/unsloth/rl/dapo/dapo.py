...
class UnslothDAPOTrainer(TrainerBase):
    ...
    # ============================================================================
    # 2. ADD _combined_reward_function() - ADD AFTER setup_rewards()
    # ============================================================================
    ...
    def setup_dataset(self) -> None:
        """Setup datasets for GRPO training using unified DataManager."""
        logger.info("Setting up TRL GRPO datasets with DataManager...")
        ...
        preserve_columns = {
            ...
            'canonical_solution', 'chosen', 'rejected'
        }

        manager = DataManager(
            task_type="grpo",  # GRPO format (query/prompt column)
        ...
        # (auto-detects columns, handles config names, splits, etc.)
        dataset_dict = manager.load_dataset(
            dataset_name,
            config_name=config_name,
            split=split
        ...
        # Extract train split (GRPO typically uses single training set)
        self.train_dataset = dataset_dict["train"]

        # Optional: use validation split if available
        self.eval_dataset = dataset_dict.get("validation", None)
        self.dataset_dict = dataset_dict

        # Filter empty prompts (additional validation)
        if len(self.train_dataset) > 0:
        ...
        logger.info(
            f"Dataset columns after processing: {
                self.train_dataset.column_names}")
        logger.info(
            f"TRL GRPO dataset loaded: {len(self.train_dataset)} train examples")
        if self.eval_dataset:
            logger.info(
        ...
        if len(self.train_dataset) > 0:
            sample = self.train_dataset[0]
            # GRPO format standardizes to 'prompt' column (DataManager
            # auto-creates 'query' alias)
            prompt_col = "query" if "query" in sample else "prompt"
            logger.info(
                f"Sample prompt (first 100 chars): {sample[prompt_col][:100]}...")

            # Verify preserved columns exist if expected
            for col in preserve_columns:
                if col in sample:
                    logger.info(f"Preserved column '{col}' found in sample")
    ...
    def setup_trainer(self) -> None:
        ...
        try:
            ...
            # Set save strategy
            save_strategy = "steps"
            # Add after: from trl import GRPOTrainer, GRPOConfig
            ...
            run_name = self._get_config_value(
                self.config.logging, 'run_name', default='unsloth_dapo')
    ...
    def save_model(self, path: Optional[str] = None) -> str:
        ...
        try:
            ...
            config_path = Path(save_path) / "dapo_training_config"
