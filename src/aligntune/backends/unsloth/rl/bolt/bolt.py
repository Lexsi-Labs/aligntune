    ...
    def setup_data(self) -> None:
        ...
        if bolt_cfg.baseline_enabled or bolt_cfg.curriculum_enabled or bolt_cfg.use_baseline_advantages:
            ...
            if bolt_cfg.baseline_warm_start:
                ...
                # Remap index-based keys to prompt-text keys if needed
                overlap = set(self.baseline.tab.keys()) & set(self._prompt_keys)
                if not overlap and self.baseline.tab and self._prompt_keys:
                    logger.info("Baseline keys don't match prompt keys, remapping by position...")
                    self.baseline.remap_keys(self._prompt_keys)
