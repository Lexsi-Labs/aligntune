...
class CounterfactualGRPOTrainer(GRPOTrainer):
    ...
    def _compute_loss_impl(self, model, inputs):
        ...
        if per_token_logps.dim() == 3 or not per_token_logps.requires_grad:
            ...
            # ===== Align ref/old logps to completion length (Unsloth shape fix) =====
            target_len = per_token_logps.size(1)
            for _key in ("ref_per_token_logps", "old_per_token_logps"):
                _t = inputs.get(_key)
                if _t is None or not isinstance(_t, torch.Tensor):
                    continue

                # Case A: Unsloth returned 3D hidden states for ref model too — convert to logps
                if _t.dim() == 3:
                    if not hasattr(self, '_unsloth_ref_3d_warning'):
                        print(f"[INFO] {_key} is 3D {tuple(_t.shape)} — computing logps from hidden states", flush=True)
                        self._unsloth_ref_3d_warning = True

                    # _t is [B, seq_len, hidden_dim] hidden states
                    # We need to apply lm_head to get logits, then gather completion logps
                    seq_len_full = _t.size(1)
                    # Try to get lm_head from the model
                    lm_head = None
                    ...
                    if lm_head is None and hasattr(model, "base_model"):
                        for attr in ("lm_head", "embed_out", "output"):
                            if hasattr(model.base_model, attr):
                                lm_head = getattr(model.base_model, attr)
                                break

                    if lm_head is None:
                        # Fallback: zero out — no KL contribution from ref
                        print(f"[WARN] Could not find lm_head, zeroing {_key}", flush=True)
                        inputs[_key] = torch.zeros(
                            _t.size(0), target_len,
                    ...
                    with torch.no_grad():
                        # Take last (target_len + 1) hidden states, drop the last (predicts off-by-one)
                        ref_hidden_completion = _t[:, -(target_len + 1):-1, :]
                        ref_logits = lm_head(ref_hidden_completion).float()
                        ref_log_probs = torch.nn.functional.log_softmax(ref_logits, dim=-1)
                        ref_logps = torch.gather(
                            ref_log_probs, dim=-1,
                            index=completion_ids.unsqueeze(-1)
                        ).squeeze(-1)
                    inputs[_key] = ref_logps
                    continue

                # Case B: 2D but wrong length — slice/pad as before
                if _t.dim() < 2 or _t.size(1) == target_len:
                    continue
                if _t.size(1) > target_len:
                    inputs[_key] = _t[:, -target_len:].contiguous()
                else:
                    _pad_shape = (_t.size(0), target_len - _t.size(1)) + tuple(_t.shape[2:])
                    _pad = torch.zeros(_pad_shape, dtype=_t.dtype, device=_t.device)
                    inputs[_key] = torch.cat([_pad, _t], dim=1).contiguous()

            if not hasattr(self, '_unsloth_align_logged'):
                print(f"[INFO] Aligned ref/old logps to completion length = {target_len}", flush=True)
                self._unsloth_align_logged = True
            # ==================================================================
