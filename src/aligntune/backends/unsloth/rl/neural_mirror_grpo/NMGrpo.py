...
class UnslothNeuralMirrorGRPOTrainer(TrainerBase):
    ...
    def is_available(cls) -> bool:
        ...
        try:
            ...
            # Unsloth is optional - will use fallback if not available
            try:
                import unsloth
                logger.info("Unsloth available - will use optimized loading")
            except ImportError:
                logger.info(
                    "Unsloth not available - will use standard loading")
    ...
    def setup_trainer(self) -> None:
        ...
        eval_steps = self._get_config_value(
            self.config.train,
            'eval_steps',
            default=100) if self.eval_dataset else None
    ...
    def setup_model(self) -> None:
        """Setup model with Unsloth optimization (same as Unsloth GRPO) but for Neural Mirror."""
        try:
            ...
            try:
                ...
                logger.info(
                    "Unsloth not available - using standard model loading")
            ...
            precision = PrecisionHandler.get_precision_from_config(
                self.config, default="auto")
            precision = PrecisionHandler.validate_precision(precision)
            PrecisionHandler.log_precision_info(
                precision, "Neural Mirror GRPO")
            ...
            if use_unsloth:
                ...
                if use_lora:
                    ...
                    lora_alpha = self._get_config_value(
                        self.config.model, 'lora_alpha', 'alpha', default=16)
                    lora_dropout = self._get_config_value(
                        self.config.model, 'lora_dropout', 'dropout', default=0)
                ...
                # Apply LoRA if specified (using standard PEFT)
                if use_lora:
            ...
            logger.error(f"Failed to setup Neural Mirror GRPO model: {e}")
            raise

    def setup_data(self) -> None:
        """Setup datasets for Neural Mirror GRPO training using unified DataManager."""
        logger.info(
            "Setting up Neural Mirror GRPO datasets with DataManager...")
        ...
        dataset_name = self._get_config_value(
            dataset_config, 'name', 'dataset_name', default='mbpp')
        ...
        # Columns to preserve for Neural Mirror specific computations
        preserve_columns = {
            'test_list', 'test', 'answer', 'solution', 'code',
            'canonical_solution', 'response', 'code_prompt'
        }
        ...
        if len(self.train_dataset) > 0:
            prompt_col = "prompt" if "prompt" in self.train_dataset.column_names else "query"
            initial_size = len(self.train_dataset)
            self.train_dataset = self.train_dataset.filter(
                lambda x: len(x[prompt_col].strip()) > 0,
                desc="Filtering empty prompts"
            )
            filtered_count = initial_size - len(self.train_dataset)
            if filtered_count > 0:
                logger.info(f"Filtered out {filtered_count} empty prompts")

        logger.info(
            f"Neural Mirror GRPO dataset loaded: {len(self.train_dataset)} train examples")
