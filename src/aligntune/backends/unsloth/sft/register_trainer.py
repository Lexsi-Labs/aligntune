"""
Unsloth Register/Formality Control Trainer for AlignTune v3.10

Unsloth-accelerated port of the TRL Register/Formality Control Trainer. Extends
TRL's SFTTrainer (unchanged) on top of an Unsloth-loaded model backbone to
support register control tokens for fine-grained tone/formality control.
Enables a single model to switch between formal, legal, technical, and simple
language registers, with Unsloth's faster training/inference optimizations.
...
class RegisterTrainerConfig:
    """Configuration for the Unsloth Register/Formality Control Trainer."""
    ...
    output_dir: str = "./output/unsloth_register"
    ...
    load_in_4bit: bool = False  # Match TRL version's full-precision default
    dtype: Optional[str] = None  # None -> let Unsloth auto-detect (bf16/fp16)
    ...
    def __post_init__(self):
        ...
        logger.info(f"RegisterTrainerConfig (Unsloth) initialized with registers: {self.registers}")


class UnslothRegisterControlledSFTTrainer(SFTTrainer):
    """
    Unsloth-accelerated SFT Trainer with register/formality control tokens.

    Extends TRL's SFTTrainer (unchanged) to:
    ...
    Intended to run on top of a model backbone loaded via Unsloth's
    `FastLanguageModel.from_pretrained` (see `UnslothRegisterTrainer`), while
    reusing TRL's `SFTTrainer` machinery as-is for the training loop.
    ...
    def __init__(self, config: RegisterTrainerConfig, *args, **kwargs):
        """
        Initialize Unsloth Register-controlled SFT Trainer.
...
class UnslothRegisterTrainer:
    """
    Wrapper class for easy Unsloth-accelerated register-controlled SFT training.

    Handles Unsloth model loading, dataset preparation, and training lifecycle.
    The model backbone is loaded via Unsloth's `FastLanguageModel.from_pretrained`
    for faster training/inference, while the actual training loop is delegated
    unchanged to TRL's `SFTTrainer` (via `UnslothRegisterControlledSFTTrainer`).
    """

    def __init__(self, config: RegisterTrainerConfig):
        """
        Initialize Unsloth Register Trainer.
        ...
        logger.info("UnslothRegisterTrainer initialized")
    ...
    def load_model_and_tokenizer(self):
        ...
        import unsloth  # noqa: F401 - ensures Unsloth's patches are applied before model load
        from unsloth import FastLanguageModel
    ...
    def setup_trainer(self, train_dataset, eval_dataset=None):
        """
        Setup UnslothRegisterControlledSFTTrainer.
        ...
        self.trainer = UnslothRegisterControlledSFTTrainer(
            config=self.config,
            model=self.model,
        ...
        logger.info("UnslothRegisterControlledSFTTrainer setup complete")
    ...
    def train(self):
        ...
        logger.info("Starting Unsloth register-controlled SFT training...")
        self.trainer.train()
