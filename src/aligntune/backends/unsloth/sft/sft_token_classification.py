"""Unsloth Token Classification Trainer (NER, POS tagging, chunking, etc.).
...
`AutoModelForTokenClassification`-style token-classification heads. This mirrors
the same limitation documented in
...
class UnslothTokenClassificationTrainer(SFTTrainerBase):
    """Unsloth-family trainer for token classification (NER, POS, etc.).

    Uses HF Trainer with AutoModelForTokenClassification under the hood.
    See module docstring for why the backbone is loaded with
    `use_unsloth=False` (Unsloth does not support token-classification heads
    via `FastLanguageModel`).
    """
    ...
    def __init__(self, config):
        ...
        self.trainer = None
        self.task_type = TaskType.TOKEN_CLASSIFICATION
        logger.info("UnslothTokenClassificationTrainer initialized")
    ...
    def setup_model(self):
        ...
        # Unsloth doesn't perfectly support TokenClassification yet (its
        ...
        if getattr(getattr(self.config.model, 'peft', None), 'enabled', False):
            ...
            self.model = adapter.apply_to_transformers(self.model, task_type="TOKEN_CLS")
    ...
    def setup_data(self):
        ...
        def tokenize(examples):
            ...
            # Align word-level labels to sub-token positions; mark non-first sub-tokens as -100
            aligned_labels = []
    ...
    def setup_training(self):
        ...
        args_dict = {
            ...
            "run_name": self._get_config_value(self.config.logging, 'run_name', default='unsloth_token_classification'),
        ...
        self.trainer = Trainer(
            ...
            data_collator=DataCollatorForTokenClassification(tokenizer=self.tokenizer, pad_to_multiple_of=8),
    ...
    def run_experiment(self):
        logger.info("Starting Unsloth token classification experiment")
        try:
