"""
Unsloth Classification Trainer - FINAL COMPLETE VERSION

Ported from backends/trl/sft/Classification_trainer.py to live alongside the
other Unsloth backends for naming/API consistency.

NOTE ON UNSLOTH ACCELERATION: This trainer loads a genuine
AutoModelForSequenceClassification / AutoModelForTokenClassification head and
drives it with the plain Hugging Face `Trainer` (NOT TRL, NOT Unsloth's
`FastLanguageModel`). Unsloth does not support sequence/token classification
heads well yet, so the underlying model loading here is intentionally kept on
standard `transformers` (use_unsloth=False in spirit) - see setup_model()
below. The class still lives under `backends/unsloth/` and exposes an
`is_available()` classmethod that checks for the `unsloth` package so callers
that route classification tasks through the Unsloth backend can probe for it,
but no Unsloth-specific fast kernels are actually applied to the classification
head itself.
"""
...
class UnslothClassificationTrainer:
    """
    Complete Unsloth-backend Classification Trainer for text and token classification.
    ...
    Unsloth does not currently accelerate AutoModelForSequenceClassification /
    AutoModelForTokenClassification heads, so this trainer loads those heads
    natively via `transformers` (use_unsloth=False) rather than through
    Unsloth's FastLanguageModel - see `is_available()` and `setup_model()`.
    """
    ...
    def __init__(self, config):
        ...
        logger.info(f"✓ UnslothClassificationTrainer initialized for {self.task_type.value}")
        print(f"✓ UnslothClassificationTrainer initialized for {self.task_type.value}")

    @classmethod
    def is_available(cls) -> bool:
        """Check if Unsloth is available.

        Note: even when Unsloth is installed, this trainer keeps its
        classification head loads on plain `transformers` (use_unsloth=False)
        because Unsloth does not support sequence/token classification heads
        well yet. This check exists purely so callers routing through the
        Unsloth backend can confirm the package is present.
        """
        try:
            import unsloth  # noqa: F401
            return True
    ...
    def setup_model(self):
        ...
        # NOTE: These loads are intentionally kept on plain `transformers`
        # (use_unsloth=False) rather than routed through Unsloth's
        # FastLanguageModel / build_model(..., use_unsloth=True) - Unsloth
        # does not support AutoModelForSequenceClassification /
        # AutoModelForTokenClassification heads well yet.
    ...
    def run_experiment(self):
        ...
        This is the main method that backend_factory calls (for the TRL
        equivalent of this trainer). Kept as `run_experiment` here for
        interface parity, in case the Unsloth backend is wired up to route
        classification tasks through this trainer in the future.
        """
        print("\n" + "=" * 80)
        print(f"UNSLOTH CLASSIFICATION TRAINING - {self.task_type.value.upper()}")
        print("=" * 80)

        logger.info(f"Starting Unsloth classification experiment: {self.task_type.value}")
...
# Export the trainer
__all__ = ['UnslothClassificationTrainer']
