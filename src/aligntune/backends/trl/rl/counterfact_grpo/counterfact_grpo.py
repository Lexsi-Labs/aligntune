...
class TRLCounterFactGRPOTrainer(TrainerBase):
    ...
    def setup_data(self) -> None:
        ...
        is_math_dataset = any(math_name in dataset_name.lower() for math_name in ['gsm8k', 'math', 'metamath', 'tatqa', 'finance_reasoning'])
        ...
        if is_math_dataset:
            ...
            if _os.path.isdir(dataset_name):
                ds = load_from_disk(dataset_name)
                if hasattr(ds, 'keys'):
                    dataset = ds[split] if split and split in ds else ds[list(ds.keys())[0]]
                elif _os.path.isdir(_os.path.join(dataset_name, split or 'train')):
                    dataset = load_from_disk(_os.path.join(dataset_name, split or 'train'))
                else:
                    dataset = ds
            else:
    ...
    def _combined_reward_function(self, completions: List[str], prompts: List[str] = None, **kwargs) -> List[float]:
        ...
        is_gsm8k = 'gsm8k' in dataset_name.lower() or 'tatqa' in dataset_name.lower() or 'finance_reasoning' in dataset_name.lower()
