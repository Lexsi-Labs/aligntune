...
class UnifiedBaseline(BaselineStore):
    ...
    def remap_keys(self, prompt_keys: list):
        """
        Remap baseline tab entries from index-based keys to prompt-text keys.

        The precompute script generates index-based keys (e.g. gsm8k_train_0,
        mbpp_train_Mbpp/601) but at runtime make_prompt_key() uses the actual
        prompt text.  This method re-keys the tab by matching entries to
        prompt_keys in insertion order (which equals dataset iteration order).
        """
        if not self.tab or not prompt_keys:
            return 0

        old_keys = list(self.tab.keys())
        n_remap = min(len(old_keys), len(prompt_keys))

        new_tab = {}
        for i in range(n_remap):
            new_tab[prompt_keys[i]] = self.tab[old_keys[i]]

        remapped = len(new_tab)
        self.tab = new_tab
        logger.info(f"Remapped {remapped} baseline entries to prompt-text keys")
        return remapped
