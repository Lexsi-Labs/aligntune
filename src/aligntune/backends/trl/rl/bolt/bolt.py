...
class BoltGRPOTrainer:
    ...
    @staticmethod
    def create_trainer_class(baseline: UnifiedBaseline, num_generations: int, prompt_key_to_idx: dict = None, prompt_text_to_task_key: dict = None, use_baseline_advantages: bool = True):
        """
        Dynamically create a GRPOTrainer subclass with baseline tracking and logging.
        ...
        Args:
            baseline: UnifiedBaseline instance for tracking
            num_generations: Number of generations per prompt
            prompt_key_to_idx: Mapping from prompt key to dataset index
            prompt_text_to_task_key: Mapping from prompt text to task key
            use_baseline_advantages: If True, replace TRL's advantages with baseline advantages.
                                    If False, still track/log but use TRL's standard advantages.
        """
        from trl import GRPOTrainer

        class _BoltGRPOTrainer(GRPOTrainer):
            """GRPOTrainer with persistent baseline tracking and optional baseline advantages."""

            def __init__(self, *args, **kwargs):
                super().__init__(*args, **kwargs)
                self._bolt_baseline = baseline
                self._bolt_num_gen = num_generations
                self._use_baseline_advantages = use_baseline_advantages
                self._prompt_key_to_idx = prompt_key_to_idx or {}
                self._prompt_text_to_task_key = prompt_text_to_task_key or {}
                self._bolt_step_count = 0
                self._last_rewards = None
                self._last_prompts = None
                # Degenerate group tracking
                self._degenerate_stats = {
                    "total_groups": 0,
                    "degenerate_groups": 0,
                    "all_pass_groups": 0,
                    "all_fail_groups": 0,
                ...
                # Log files
                self._sampling_log_file = None
                self._degenerate_log_file = None
                self._baseline_state_log_file = None
            ...
            def _generate_and_score_completions(self, inputs):
                ...
                for i in range(batch_size):
                    group_r = rewards_reshaped[i]
                    unique_rewards = torch.unique(group_r)
                    if len(unique_rewards) == 1:
                        step_degenerate += 1
                        if unique_rewards[0].item() >= 0.99:
                            step_all_pass += 1
                        elif unique_rewards[0].item() <= 0.01:
                            step_all_fail += 1

                self._degenerate_stats["total_groups"] += batch_size
                self._degenerate_stats["degenerate_groups"] += step_degenerate
                self._degenerate_stats["all_pass_groups"] += step_all_pass
                self._degenerate_stats["all_fail_groups"] += step_all_fail

                # Get prompt keys for baseline lookup
                # Prefer task_id from inputs (direct match to baseline keys)
                prompt_keys = []

                # inputs can be a dict or list depending on TRL version
                if isinstance(inputs, dict):
                    task_ids = inputs.get('task_id', [])
                elif isinstance(inputs, list) and len(inputs) > 0 and isinstance(inputs[0], dict):
                    task_ids = [item.get('task_id') for item in inputs if isinstance(item, dict) and 'task_id' in item]
                else:
                    task_ids = []

                if task_ids and len(task_ids) >= batch_size:
                    # Use task_id directly - most reliable method
                    for i in range(0, len(task_ids), num_gen):
                        if i < len(task_ids):
                            task_id = task_ids[i]
                            if isinstance(task_id, str) and task_id.startswith("Mbpp/"):
                                key = f"mbpp_train_{task_id}"
                            else:
                                key = f"mbpp_train_Mbpp/{task_id}"
                            prompt_keys.append(key)
                    prompt_keys = prompt_keys[:batch_size]
                else:
                    # Fallback: use prompt text mapping or hash-based key
                    for i in range(0, len(prompts_text), num_gen):
                        if i < len(prompts_text):
                            prompt_text = prompts_text[i]
                            task_desc = prompt_text.strip().lower()[:200]
                            if task_desc in self._prompt_text_to_task_key:
                                key = self._prompt_text_to_task_key[task_desc]
                            else:
                                key = make_prompt_key(prompt_text)
                            prompt_keys.append(key)
                    prompt_keys = prompt_keys[:batch_size]
                    if self._bolt_step_count == 1:
                        logger.info("task_id not available in batch, using prompt text mapping for baseline lookup")
                ...
                with torch.no_grad():
                    for i, key in enumerate(prompt_keys):
                        ...
                        new_v_hats[key] = self._bolt_baseline.get_v_hat(key)

                # === DETAILED LOGGING FILES ===
                if self._sampling_log_file is None:
                    log_path = os.path.join(self.args.output_dir, "sampling_log.jsonl")
                    self._sampling_log_file = open(log_path, "w")
                if self._degenerate_log_file is None:
                    degen_path = os.path.join(self.args.output_dir, "degenerate_log.jsonl")
                    self._degenerate_log_file = open(degen_path, "w")
                if self._baseline_state_log_file is None:
                    baseline_path = os.path.join(self.args.output_dir, "baseline_state_log.jsonl")
                    self._baseline_state_log_file = open(baseline_path, "w")

                # Log each prompt's data
                for i, key in enumerate(prompt_keys):
                    group_rewards = rewards_reshaped[i].tolist()
                    prompt_idx = self._prompt_key_to_idx.get(key, hash(key) % 100000)
                    log_entry = {
                        "step": self._bolt_step_count,
                        "prompt_idx": prompt_idx,
                        "prompt_key": key[:50],
                        "rewards": [round(r, 3) for r in group_rewards],
                        "mean_reward": round(np.mean(group_rewards), 3),
                        "old_v_hat": round(old_v_hats[key], 3),
                        "new_v_hat": round(new_v_hats[key], 3),
                        "delta_v": round(new_v_hats[key] - old_v_hats[key], 4),
                        "is_degenerate": len(set([round(r, 2) for r in group_rewards])) == 1,
                    }
                    self._sampling_log_file.write(json.dumps(log_entry) + "\n")

                # Degenerate group log
                all_v_hats_old = list(old_v_hats.values())
                all_v_hats_new = list(new_v_hats.values())
                global_v_hats = [self._bolt_baseline.get_v_hat(k) for k in list(self._bolt_baseline.tab.keys())[:1000]]

                adv_std_val = advantages_normalized.std().item()
                reward_mean_all = rewards_reshaped.mean().item()
                success_rate_all = (rewards_reshaped > 0.5).float().mean().item()

                degenerate_entry = {
                    "step": self._bolt_step_count,
                    "batch_size": batch_size,
                    "degenerate": step_degenerate,
                    "informative": batch_size - step_degenerate,
                    "degenerate_rate": round(step_degenerate / batch_size, 4) if batch_size > 0 else 0,
                    "all_pass": step_all_pass,
                    "all_fail": step_all_fail,
                    "advantage_std": round(adv_std_val, 4),
                    "reward_mean": round(reward_mean_all, 4),
                    "success_rate": round(success_rate_all, 4),
                    "cumulative_degenerate": self._degenerate_stats["degenerate_groups"],
                    "cumulative_total": self._degenerate_stats["total_groups"],
                }
                self._degenerate_log_file.write(json.dumps(degenerate_entry) + "\n")

                # Baseline state log
                batch_v_mean = np.mean(all_v_hats_new) if all_v_hats_new else 0.5
                batch_v_std = np.std(all_v_hats_new) if all_v_hats_new else 0
                global_v_mean = np.mean(global_v_hats) if global_v_hats else 0.5
                baseline_entry = {
                    "step": self._bolt_step_count,
                    "batch_v_hat_new": {"mean": round(batch_v_mean, 4), "std": round(batch_v_std, 4)},
                    "batch_mean_reward": round(np.mean(mean_rewards_per_prompt), 4),
                    "global_v_hat": {"mean": round(global_v_mean, 4), "num_tracked": len(self._bolt_baseline.tab)},
                }
                self._baseline_state_log_file.write(json.dumps(baseline_entry) + "\n")

                if self._bolt_step_count % 10 == 0:
                    self._sampling_log_file.flush()
                    self._degenerate_log_file.flush()
                    self._baseline_state_log_file.flush()

                # Console logging
                degen_pct = 100 * step_degenerate / batch_size if batch_size > 0 else 0
                batch_reward = np.mean(mean_rewards_per_prompt) if mean_rewards_per_prompt else 0
                print(f"[Step {self._bolt_step_count:4d}] Degen: {step_degenerate}/{batch_size} ({degen_pct:5.1f}%) | "
                      f"Pass:{step_all_pass} Fail:{step_all_fail} | "
                      f"v̂_batch: {batch_v_mean:.3f}±{batch_v_std:.3f} | "
                      f"v̂_global: {global_v_mean:.3f} | "
                      f"r̄: {batch_reward:.3f}")
                ...
                if self._bolt_step_count % 50 == 0:
                    ...
                    total_groups = self._degenerate_stats["total_groups"]
                    total_degen = self._degenerate_stats["degenerate_groups"]
                    degen_rate = 100 * total_degen / total_groups if total_groups > 0 else 0
                    ...
                    logger.info(f"  DEGENERATE GROUPS (cumulative): {total_degen}/{total_groups} ({degen_rate:.1f}%)")

                # 5. CONDITIONALLY REPLACE TRL's advantages with ours
                if self._use_baseline_advantages:
                    output["advantages"] = advantages_normalized
                else:
                    logger.debug(f"Step {self._bolt_step_count}: Using TRL's standard advantages")
...
- No markdown, no explanations, no examples"""
    ...
    def setup_data(self) -> None:
        ...
        # Build prompt_key -> index mapping for detailed logging
        # Also build mapping from formatted prompt text to task_id based key for baseline lookup
        self._prompt_key_to_idx = {}
        self._prompt_text_to_task_key = {}

        # Detect dataset type from config
        dataset_name = self.config.datasets[0].name.lower() if self.config.datasets else ""
        is_gsm8k = "gsm8k" in dataset_name
        is_mbpp = "mbpp" in dataset_name

        for idx, item in enumerate(self._base_dataset_list):
            prompt = item.get("prompt") or item.get("query") or item.get("question") or item.get("text") or ""

            # Use task_id for baseline key if available (matches precomputed baseline format)
            ...
            if task_id:
                if isinstance(task_id, str) and task_id.startswith("Mbpp/"):
                    ...
                    key = f"mbpp_train_Mbpp/{task_id}"
                # Build mapping from task description to baseline key
                task_desc = prompt.strip().lower()[:200]
                self._prompt_text_to_task_key[task_desc] = key
            elif is_gsm8k:
                key = f"gsm8k_train_{idx}"
                task_desc = prompt.strip().lower()[:200]
                self._prompt_text_to_task_key[task_desc] = key
            elif is_mbpp:
                key = f"mbpp_train_{idx}"
                task_desc = prompt.strip().lower()[:200]
                self._prompt_text_to_task_key[task_desc] = key
            else:
            ...
            self._prompt_key_to_idx[key] = idx

        logger.info(f"Extracted {len(self._prompt_keys)} prompt keys")
        logger.info(f"Built prompt_text->task_key mapping with {len(self._prompt_text_to_task_key)} entries")
        logger.info(f"Built prompt_key->idx mapping with {len(self._prompt_key_to_idx)} entries")
        ...
        if bolt_cfg.baseline_enabled or bolt_cfg.curriculum_enabled or bolt_cfg.use_baseline_advantages:
            ...
            if bolt_cfg.baseline_warm_start:
                ...
                # Check key overlap
                overlap = set(self.baseline.tab.keys()) & set(self._prompt_keys)
                if overlap:
                    logger.info(f"Baseline keys match prompt keys: {len(overlap)} overlap")
                elif self.baseline.tab and self._prompt_keys:
                    # Fallback: remap by position if keys don't match
                    logger.warning("Baseline keys don't match prompt keys, remapping by position...")
                    n = self.baseline.remap_keys(self._prompt_keys)
                    logger.info(f"Remapped {n} baseline entries to prompt-text keys")
    ...
    def setup_trainer(self) -> None:
        ...
        # Ensure eval batch size is divisible by num_generations (GRPOConfig requirement)
        if num_generations and per_device_eval_batch_size % num_generations != 0:
            per_device_eval_batch_size = max(num_generations, (per_device_eval_batch_size // num_generations) * num_generations)
        ...
        grpo_config = GRPOConfig(
            ...
            epsilon=epsilon,
            loss_type="grpo",
            num_iterations=4,  # Train 4 times per batch (default 1 = plain REINFORCE)
            ...
            **precision_args,
            ...
            # Drop incomplete batches at epoch boundary to avoid batch_size=1 mini-batches
        ...
        if bolt_cfg.use_baseline_advantages and self.baseline is not None:
            ...
            TrainerClass = BoltGRPOTrainer.create_trainer_class(
                baseline=self.baseline,
                num_generations=num_generations,
                prompt_key_to_idx=getattr(self, '_prompt_key_to_idx', None),
                prompt_text_to_task_key=getattr(self, '_prompt_text_to_task_key', None),
