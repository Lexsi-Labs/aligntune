...
class TRLDAPOTrainer(TrainerBase):
    ...
    def setup_trainer(self) -> None:
        """Setup the GRPO trainer with all configurations."""
        from trl import GRPOTrainer, GRPOConfig
        ...
        save_strategy = self._get_config_value(
            self.config.train, 'save_strategy', default='epoch')
        ...
        grpo_config = GRPOConfig(
            ...
            per_device_eval_batch_size=per_device_batch_size,  # Reuse train batch size
            ...
            **precision_args,  # Unpacks fp16=True/False, bf16=True/False
        )
        # Create GRPO trainer based on environment
        ...
        if should_use_pure_trl:
            ...
            try:
                ...
                def unsloth_reward_wrapper(
                    ...
                    return self._combined_reward_function(completions)
        ...
        try:
            if isinstance(self.config.logging, dict):
                ...
                default_path = getattr(self.config.logging,
                                       'output_dir',
                                       './output/dapo') if hasattr(self.config,
                                                                   'logging') else './output/dapo'
            ...
            config_path = Path(save_path) / "dapo_training_config.yaml"
