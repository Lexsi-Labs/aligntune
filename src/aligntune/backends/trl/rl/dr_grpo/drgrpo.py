...
class TRLDRGRPOTrainer(TrainerBase):
    ...
    def setup_trainer(self) -> None:
        ...
        output_dir = self._get_config_value(self.config.logging, 'output_dir', default='./output/drgrpo_trl')
        
        # GRPO-specific parameters (beta = KL coefficient, epsilon = clip range)
        ...
        num_generations = self._get_config_value(self.config.train, 'num_generations', default=4)
        max_completion_length = self._get_config_value(self.config.train, 'max_completion_length', 'max_new_tokens', default=256)
    ...
    #     logger.info("Starting TRL GRPO evaluation...")
        ...
        try:
            if isinstance(self.config.logging, dict):
                default_path = self.config.logging.get(
                    'output_dir', './output/drgrpo')
            else:
                default_path = getattr(self.config.logging,
                                       'output_dir',
                                       './output/drgrpo') if hasattr(self.config,
                                                                   'logging') else './output/drgrpo'
