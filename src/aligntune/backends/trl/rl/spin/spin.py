"""
TRL SPIN Trainer - Self-Play Fine-Tuning Implementation
...
class TRLSPINTrainer(TrainerBase):
    """
    SPIN Trainer using TRL's DPOTrainer internally.
    ...
    def is_available(cls) -> bool:
        """Check if TRL DPO trainer is available for SPIN."""
        try:
    ...
    def setup_model(self) -> None:
        """Setup model and tokenizer for SPIN via unified model_loader."""
        try:
            from aligntune.core.model_loader import build_model
            from aligntune.core.registry import TaskType

            logger.info("=" * 80)
            logger.info("Setting up TRL SPIN models via model_loader")
            logger.info("=" * 80)
            ...
            logger.info("TRL SPIN model setup completed successfully")

        except Exception as e:
            logger.error(f"Failed to setup TRL SPIN model: {e}")
            raise
    ...
    def setup_trainer(self) -> None:
        """Setup DPO trainer for SPIN rounds."""
        try:
            from trl import DPOTrainer, DPOConfig

            logger.info("Setting up DPO trainer for SPIN rounds")
            ...
            # Backfill anything else the caller set (e.g. weight_decay,
            # precompute_ref_log_probs, f_divergence_type, sync_ref_model, ...)
            # that the explicit list above doesn't already cover. Fields SPIN
            # deliberately overrides above (save_steps, max_steps) are protected
            # since their value already differs from DPOConfig's own default.
    ...
    def generate_responses(
        ...
        try:
            ...
            # Keep decoded text on CPU.  Leaving the full generation tensor
            # on GPU until the next batch can unnecessarily grow the CUDA
            # allocator's reserved memory, especially with long completions.
            ...
            if messages_mode:
                ...
                for row in source_dataset:
                    ...
                    if getattr(self.tokenizer, "chat_template", None):
                        ...
                        # The dataset may carry this value, but the trainer
                        # configuration is authoritative for SPIN generation.
                        ...
                        try:
                            ...
                            # Older/custom templates may not expose the
