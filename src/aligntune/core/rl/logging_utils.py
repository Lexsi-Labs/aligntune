...
class UnifiedLogger:
    ...
    def log_reward_breakdown(self, step: int, breakdown_dict: Dict[str, float]) -> None:
        """Log per-component reward breakdown (rank-0 only).

        Args:
            step: Training step
            breakdown_dict: Dict mapping reward component name to its value
        """
        if not self.is_rank_0:
            return

        # Prefix all rewards
        prefixed_breakdown = {f"rewards/{name}": value for name, value in breakdown_dict.items()}
        ...
        # Log to TensorBoard
        if self.tensorboard_logger:
            for key, value in prefixed_breakdown.items():
                self.tensorboard_logger.add_scalar(key, value, step)

        # Log to WandB
        if self.wandb_logger:
            try:
                self.wandb_logger.log(prefixed_breakdown, step=step)

                # Also log as a table row if available
                if HAS_PANDAS:
                    table_rows = []
                    for reward_name, value in breakdown_dict.items():
                        table_rows.append({
                            "step": step,
                    ...
                    if table_rows:
                        ...
                        self.wandb_logger.log({"reward_breakdown_table": table}, step=step)
            except Exception as e:
                logger.warning(f"Failed to log reward breakdown to WandB: {e}")

        # Log to console
        breakdown_str = ", ".join([f"{k}: {v:.4f}" for k, v in breakdown_dict.items()])
        logger.info(f"Step {step} - Reward breakdown: {breakdown_str}")
